...
Vector3 Basis::get_euler(EulerOrder p_order) const {
	...
	return Vector3();
} // could use a simd acceleration

void Basis::set_euler(const Vector3 &p_euler, EulerOrder p_order) {
    // Use SIMD-enabled methods for computing sine and cosine in bulk
    Vector3 sin_vals = Vector3::sin_vec(p_euler.x, p_euler.y, p_euler.z, 0.0f);
    Vector3 cos_vals = Vector3::cos_vec(p_euler.x, p_euler.y, p_euler.z, 0.0f);

    real_t sx = sin_vals.x, sy = sin_vals.y, sz = sin_vals.z;
    real_t cx = cos_vals.x, cy = cos_vals.y, cz = cos_vals.z;
